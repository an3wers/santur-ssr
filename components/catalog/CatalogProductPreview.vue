<template>
  <div
    class="product-preview flex flex-col sm:flex-row space-y-2 sm:space-y-0 space-x-0 sm:space-x-4 pt-4 first:pt-0"
  >
    <div class="product-preview__image shrink-0 w-24 h-24">
      <img
        :src="product.imgpath"
    ...
    <div class="product-preview__body grow">
    ...
    <div
      class="product-preview__properies flex flex-col flex-nowrap shrink-0 w-56 space-y-3"
    >
...
async function addCartHandler() {
  ...
  const prodTmp = categoryStore.categoryProducts.find(
...
async function setNewValue() {
  ...
  if (props.product.qty_incart != productAmountInCart.value) {
    ...
    // console.log('value', productAmountInCart.value)
...
async function addFavorityHandler(product) {
  const current = categoryStore.categoryProducts.find(
    (el) => el.code === product
  );
  await profileStore.addToFavorities(product);
  current.isfavority = true;
}
